...
<template>
  <section class="relative min-h-screen flex flex-col items-center justify-center overflow-hidden bg-black">
    ...
    <div class="absolute inset-0 pointer-events-none">
      <div class="absolute top-1/4 left-1/4 w-[600px] h-[600px] rounded-full blur-[120px] opacity-10"
        style="background: radial-gradient(circle, #00BFFF, transparent 70%)" />
      <div class="absolute bottom-1/4 right-1/4 w-[500px] h-[500px] rounded-full blur-[120px] opacity-10"
    ...
    <div class="relative z-10 text-center px-6 max-w-6xl mx-auto hero-content">
      <div class="inline-flex items-center gap-2 px-4 py-2 rounded-full glass border border-white/10 mb-8">
        ...
        <span class="text-xs tracking-widest text-white/60 uppercase font-mono">{{ t('hero.badge') }}</span>
      </div>

      <h1 ref="titleRef"
        class="text-[clamp(4rem,14vw,12rem)] font-bold leading-none tracking-tighter mb-6 transition-transform duration-200"
        style="font-family: Syne, sans-serif" @mousemove="onTitleMove" @mouseleave="onTitleLeave">
      ...
      <div class="flex flex-col sm:flex-row items-center justify-center gap-4">
        <MagneticButton href="https://open.spotify.com/artist/6hkNwIjIfDcMDp5AObEbO9" target="_blank" variant="primary"
          size="lg">
        ...
        <MagneticButton href="/services" variant="outline" size="lg">{{ t('hero.order') }}</MagneticButton>
        <MagneticButton href="/contact" variant="ghost" size="lg">{{ t('hero.contact') }}</MagneticButton>
      </div>
    </div>

    <div
      class="absolute bottom-10 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 text-white/30 scroll-hint">
...
<style scoped>
.hero-content {
  animation: heroIn 0.9s cubic-bezier(0.76, 0, 0.24, 1) 2s both;
}

.scroll-hint {
  animation: fadeIn 0.6s ease 3s both;
}
